...
async def read_accounts(accounts: AccountRepository = Depends(get_account_dal)):
    return paginate(await accounts.get_all())


@router.get('/{account_id}', response_model=schemas.AccountWithoutAgents)
async def read_account(account_id: UUID, accounts: AccountRepository = Depends(get_account_dal)):
...
async def generate_company_report(account_id: UUID, accounts: AccountRepository = Depends(get_account_dal)):
...
async def delete_account(account_id: UUID, accounts: AccountRepository = Depends(get_account_dal),
...
async def clear(_two_fa_token: Any = Depends(get_2fa_token_header), accounts: AccountRepository = Depends(get_account_dal),
                rabbit_producer: EventProducer = Depends(get_event_producer)):
